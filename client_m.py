...
def select_data(data, n, i):
    ...
    selected_fraction_X, selected_fraction_y = fractions[i - 1]  # Access the i-th fraction (1-based)
...
def evaluateLocalModel(model, test_loader):
    ...
    with torch.no_grad():  # Disable gradient computation (optimisation)
        for X_batch, y_batch in test_loader:
            ...
            labels.extend(y_batch.cpu().numpy())  # Move to CPU and convert to numpy
...
def attack_IPM(weights, train_loader, byz_count, epsilon=15): # ensure that epsilon > K/M - 1 | 21/19 - 1 for a change of sign
    ...
    for name, param in weights.items():
        if name in avg_honest_gradient:
            # Calculate poisoned update
            poisoned_weights[name] = weights[name] - epsilon * avg_honest_gradient[name]
        else:
...
def main(local_id, atk_opt):
    ...
    atk_opt - attack option, chooses which attack to go with when injecting model poisoning.
    {0: gaussian noise, 1: ALIE, 2: IPM}
    """
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # Unpack received user key and master public key
        #response_uk_mpk = s.recv(1024)
        ...
        s.recv(1024) # ACK SEQUENCE

        s.sendall(b'ACK ACK')  # Acknowledge completion of authentication
